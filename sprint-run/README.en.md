...
1. **Bash permission auto-allow.** `.claude/settings.local.json` carries an **allowlist** with the
   ...
   template.
2. **Worktree as safety boundary.** `execution_isolation` must be `worktree` — otherwise **abort**.
...
2. **Check safety prerequisites.** Allowlist present? `execution_isolation=worktree`? Bodyguard
...
- **`.claude/settings.local.json` with a gate allowlist** (created by `/bootstrap`) and
  `execution_isolation=worktree` in `CONVENTIONS.md` — both are prerequisites for the `/goal` call
...
| `execution_isolation` | Must be `worktree` (safety prerequisite) | `worktree` |
