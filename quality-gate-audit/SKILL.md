...
| `pre-sprint` | **HARD-HOOK** in `/sprint-run` Schritt 1 | alle `verdrahtet` → Sprint startet; ≥1 `blind` → **STOPP**. Override transient via `--override-gate … --reason` **oder** persistent via Report-Frontmatter `override_blind: true` + `reason`. Daemon (`--auto`) ohne Override → Abbruch. |
...
- **Signal-Tests:** [scripts/signal-tests/](scripts/signal-tests/) — `bodyguard-canary-edit.sh`
  (synthetischer AWS-Key-Edit gegen den Bodyguard-Hook). Der Semgrep-Canary-Scan laeuft direkt in
  `gate-checks.sh`.
