...
> 🧑‍💻 **Schnell-Nachschlagewerk:** Die kompakte „Wann nehme ich was?"-Übersicht (≤5 Seiten, DE+EN) steht im [Entwickler-Schnelleinstieg](docs/quickstart-entwickler.md) — dieser Abschnitt ist die ausführliche Fassung.
...
## 11b. Checkpoints & Rollback per /rewind (BOO-208)

Anthropics `/rewind` (`Esc Esc`) spult **Claude-Edits innerhalb der laufenden Session** zurück — ein natives Sicherheitsnetz, das das Framework nutzt statt nachbaut (Build-vs-Buy, ADR-1).

- **Checkpoint setzen:** Vor jedem `git commit` einen Checkpoint setzen, falls die Story einen Rollback brauchen könnte. So lässt sich ein missglückter Implementierungs-Schritt verwerfen, ohne den Git-Verlauf anzufassen.
- **Grenze:** `/rewind` betrifft **nur Claude-Edits**. NICHT erfasst: manuelle Änderungen, Bash-/Tool-Outputs sowie bereits committete oder gepushte Stände — dafür bleibt Git (`git revert`, `git reset`) das Mittel.
- **Abgrenzung zum Rollback-Marker:** Der Marker `// AI-generated: {STORY_ID}` (BOO-17) *identifiziert* AI-Code nachträglich; `/rewind` *spult* aktiv in-Session zurück. Beide ergänzen sich.

Details im Skill: `/implement` Schritt 5 (Sektion „Rollback per `/rewind`").
...
| **AQ** | Status Line — Worker-Equivalent live | native Claude-Code-Status-Line: Modell, Token-Budget, Worker-Equivalent (h/h) und Story live im Prompt; Verdrahtung in `settings.json`, nur bei `runtime_target=claude-code` (Schalter A) |
...
### Warum Hermes nur Vorschlaege macht (Human-in-the-Lead)

Der Hermes-Layer ist bewusst passiv. Er liest Sprint-Retros, CI-Reports und SonarQube-Trends und schlaegt Skill-Verbesserungen als Pull Requests vor. Uebernommen werden sie erst nach menschlicher Pruefung. Wir wenden Verbesserungen nicht automatisch an.

Der Grund ist Drift in Selbstverbesserungs-Schleifen. Drei Mechanismen sind belegt:

- Model Collapse: Lernt ein Modell wiederholt auf selbst-erzeugten Daten, degeneriert es ueber Generationen, bis die Raender der urspruenglichen Verteilung verschwinden ([Shumailov et al., Nature 2024](https://www.nature.com/articles/s41586-024-07566-y)).
- Catastrophic Forgetting: Nachtrainieren auf Neues verschlechtert messbar Altes, in Agenten als Regression bei stabilen Tool-Use-Mustern und als Memory-Drift ([Hintergrund](https://www.infoworld.com/article/4131242/researchers-propose-a-self-distillation-fix-for-catastrophic-forgetting-in-llms.html)).
- Skill-Drift in Agenten-Skill-Bibliotheken: rekursive Skill-Updates ohne Pruefung koennen die Bibliothek verschlechtern ([arXiv](https://arxiv.org/html/2604.20087)).

Hermes' Learning-Loop trainiert keine Modellgewichte, sondern pflegt Skills und Memory. Trotzdem greift Drift auf Ebene der Skill-Bibliothek. Ein menschlicher Review-Gate bricht die rekursive Schleife. Deshalb: Hermes schlaegt vor, der Operator entscheidet. Das deckt sich mit dem Approval-Gate aus Anhang F (Pflicht, nie deaktivieren) und mit ADR-5 (Hermes-Layer vs. Agent Teams).
...
> 📄 **Ausführliches Runbook:** Mechanik (Auslöser → Codex-Sandbox → `memory.md` → Findings), `.toml`-Felder, Setup, Datenschutz-Tier und Grenzen stehen im [Daily-Bug-Scanner-Runbook](docs/runbooks/daily-bug-scanner.md) (DE+EN). Produktisierung als Opt-in-Layer: BOO-239.
...
- **BOO-191** — Worker-Equivalent-Report im `/sprint-review` (Schritt 9b, nach dem ccusage-Snapshot). Er ist die **Quelle des Ist-ROI je Sprint**: liest den aktiven Satz aus der Baseline (Abschnitt 1), aggregiert Σ `effort_human_equiv_hours` (× Satz = Mensch-Equiv-Kosten) und Σ `effort_ai_hours` (nur Kontext), zieht die KI-Kosten aus dem Cost-Aggregat/ccusage und legt **ROI = Mensch-Equiv-Kosten ÷ KI-Kosten** plus Wall-Clock in `docs/financials/sprint-XX-worker-equivalent.md` ab (native Währung, graceful skip ohne Baseline/Doppelspalte). BOO-192 (Forecast) liest dieses Ist-Schema maschinell gegen.
- **BOO-192** — Sprint-Forecast im `/backlog` (Schritt 4b). Das **Forecast-Gegenstueck** zum Ist-Report: pro geplanter Story schaetzt `/backlog` die erwarteten KI-Kosten (`token_estimate` × Tier-Preis aus `model-tiers.json` ueber `recommended_model`), den Mensch-Equiv-Wert (`effort_human_equiv_hours` × Baseline-Satz), die Wall-Clock (Heuristik aus `effort_ai_hours`) und den ROI-Faktor; ueber alle Stories zum **Sprint-Aggregat**. Der Forecast wird unter `docs/financials/sprint-XX-forecast.md` persistiert — **feld-gleiches** Frontmatter wie der Ist-Report (`type: worker-equivalent-forecast`). Beim naechsten Planungslauf liest `/backlog` (Schritt 4b-2) den Forecast gegen den Ist-Report und weist die **Forecast-vs-Ist-Drift** je Dimension aus (`ki_cost`, `human_equiv_cost`, `roi_factor`). Die Drift ist ein **Quality-Gate-Signal** (Kalibrierung, Anschluss `quality-gate-audit` Cost-Drift, vgl. [[Decisions/2026-05-06 Cost-Drift als Quality-Gate-Dimension]]) — **kein Hard-Block**. Native Waehrung, keine FX; graceful skip ohne Baseline/Doppelspalte.
...
## Anhang AN: Cloud-Engine /ultraplan (BOO-207)

> Quelle: ADR-2 Schalter B (`prefer_ultraplan`). Schwester zu Anhang AH (`/goal`): `/goal` **terminiert**, `/ultraplan` **plant auf Code-Ebene**.

`/ultraplan` ist Anthropics native **Code-Level-Planungs-Engine**. `/architecture-review` und `/ideation` schlagen sie vor, statt sie nachzubauen (Build-vs-Buy, ADR-1).

**Vorteil-Hierarchie** — wer beantwortet was:

| Ebene | Artefakt | Frage |
|---|---|---|
| Linear-Story | Issue | WAS (Outcome) |
| Spec | `specs/<story>.md` | WIE auf Skill-Ebene |
| Sprint-Plan | `Sprints.md` | REIHENFOLGE |
| Ultraplan | `## Plan` in der Spec | CODE-LEVEL (Phasen, Schemata, Migration, Konflikt-Risiko) |

**Schalter `prefer_ultraplan`** (im `native_paths`-Block der CLAUDE.md, BOO-199):

- `auto` (Default) — Vorschlag nur bei Story `>5 SP` ODER Architektur-Bruch (neue Schicht/Pattern, Migration mit Konfliktpotenzial, Cross-Cutting-Change).
- `always` — immer vorschlagen. `never` — nie.
- Nur aktiv bei `runtime_target: claude-code` (Schalter-A-Kopplung).

**Output** landet in der `## Plan`-Sektion von `specs/<story>.md` (ergänzt ADD/Phasen-Plan, ersetzt sie nicht).

**Beispiel** (dpo-Kontrollkatalog als YAML): Ultraplan liefert „Phase A: YAML-Schema, Phase B: Migration-Script, Phase C: Loader. Konflikt-Risiko: 47 Referenzen in nDSG-Prosa müssen mitmigrieren."

Getriggert in `/architecture-review` (Schritt 0b) und `/ideation` (Schritt 3b).

---

## Anhang AO: Sprint Review vs /insights — komplementäre Native-Features (BOO-211)

> Quelle: ADR-2 Schalter B (`complement_insights`) + ADR-3 (Automemory vs Learning-Loop). `/sprint-review` und `/insights` klingen ähnlich, beantworten aber verschiedene Fragen — dieser Anhang grenzt sie ab.
...
**Wie `complement_insights: true` beide kombiniert:** `/sprint-review` schreibt den Learning-Loop-Eintrag (Schritt 8); bei aktivem Flag stößt Schritt 7d zusätzlich den Skill `insights-review` an, der `/insights` aufruft und einen **abgegrenzten** Operator-Reflexions-Meta-Block voranstellt — ohne ihn mit dem Learning-Loop zu vermischen (ADR-3). Beispiel-Ausschnitt aus `journal/sprint-{date}.md`:

```markdown
## Operator-Reflexion (/insights — complement_insights)
> Operator-Arbeitsmuster — lokal, kein Projekt-Wissen (ADR-3).
- Arbeitsmuster: viele kurze Implement-Iterationen, wenig Batch
- Reibungspunkte: Kontext-Reload nach jedem Sprint-Wechsel
- Nächster Operator-Change: Specs-Bündel vor Sprint-Start lesen

## Learnings — Sprint {date} (Learning-Loop, Projekt-Wissen)
### Was funktionierte
- Native-Subagent-Migration (BOO-204) sauber gelandet
```
...
## Anhang AQ: Status Line — Worker-Equivalent live (BOO-205)

> **Kurz:** Eine native Claude-Code-Status-Line zeigt Modell, Token-Budget, Worker-Equivalent und Story-Budget **live** im Prompt. Build-vs-Buy (ADR-1, Anhang AG): native Status-Line statt Eigenbau-UI. Aktiv nur bei `runtime_target: claude-code` (Schalter A, Anhang AL).

**Was sie zeigt** (eine Zeile, Felder mit ` | ` getrennt, leere weggelassen):
...
| Feld | Quelle | Art |
|------|--------|-----|
| Modell | `.model.display_name` (stdin) | nativ |
| `Nk/Mk ctx` | `.context_window.total_input_tokens` / `.context_window_size` (stdin) | nativ |
| `Worker-Equiv: Xh / Yh` | `effort_ai_hours` / `effort_human_equiv_hours` aus `specs/<story>.md` (BOO-193) | Framework-Anreicherung |
| `Story BOO-<n>` | `.worktree.branch` bzw. `git branch` | nativ/abgeleitet |
| `$X.XX` | `.cost.total_cost_usd` (stdin) | nativ — Schattenpreis (Max-Pro) / Echtkosten (API) |

**Warum custom?** Modell, ctx und Kosten liefert Claude Code bereits nativ auf stdin. Der **eine** Mehrwert, der die Custom-Status-Line rechtfertigt, ist das **Worker-Equivalent** (ADR-D, Anhang AK): die zweite Steuerungs-Dimension neben Token, gelesen aus der Story-Spec — sonst nur im Sprint-Report sichtbar, jetzt live im Moment der Arbeit.

**Verdrahtung.** Claude Code liest die Status-Line **ausschliesslich** aus `.claude/settings.json`:
...
`.claude/environment.json` ist das Framework-**Manifest**, nicht der native Render-Pfad — `migrate_boo_205` hinterlegt dort zusaetzlich `"status_line": ".claude/statusline.sh"` als Referenz (das Manifest kennt das Skript; gerendert wird ueber `settings.json`).
...
## Anhang AU: Unabhängiger Code-Reviewer — Codex als Opt-in-Layer (BOO-239)

> **Kurz:** Ein **unabhängiger** Code-Reviewer — bewusst ein *anderes* Tool als der Autor (Claude): **Codex**. Prüft Codequalität aus Senior-Dev-Sicht, **schlägt nur vor** (ändert nie Code), **opt-in, kein Gate**. Reviewer ≠ Autor = CoI-Control. Setup: `docs/runbooks/codex-reviewer.md`.

**Warum ein anderer Anbieter.** Lässt man den Autor seinen eigenen Code reviewen, ist das ein Interessenkonflikt — dieselben blinden Flecken, dieselbe Modell-Familie. Ein Reviewer von einem *anderen* Anbieter (Codex statt Claude) ist die Vier-Augen-Kontrolle auf Werkzeug-Ebene (vgl. BOO-150, Human-in-the-Lead BOO-227). Er ergänzt die deterministischen Gates (Semgrep/Sonar/Coverage) um einen LLM-Senior-Dev-Pass, der Design/Handwerk/Edge-Cases fängt, die Regeln nicht sehen.

**Report-only — die Behebung bleibt im Framework.** Codex ändert nie Code. Ein echter Befund wird **triagiert → Backlog-Story + Spec → `/implement` → Gates → menschlicher Merge**. So durchläuft die *Behebung* die Governance, nicht ein roher Codex-Patch (kein Auto-Fix, kein Auto-Merge, BOO-227).

**Zwei Tiers (Reviewer-Tier folgt Repo-Tier).** Komfort (80 %): native GitHub-PR-Review, Code → OpenAI-Cloud (US). Souveränität (EU/CH): Codex-CLI selbst betrieben gegen Azure-OpenAI-EU (Sweden Central / West Europe / Germany West Central — **nicht** Switzerland North = nur GPT-4o). Die native Cloud-Review ist **nicht** EU-residenzfähig. Details, `config.toml` und die zwei Datensprung-Sketches: `docs/runbooks/codex-reviewer.md`.

**AGENTS.md als Review-Vertrag.** Codex reviewt gegen die Projekt-Artefakte (SECURITY.md, ARCHITECTURE_DESIGN.md, CONVENTIONS.md) über die AGENTS.md-Review-Guidelines — nicht generisch. Template in `bootstrap/references/file-templates.md`, Bestandsprojekte via `migrate_boo_239`.
